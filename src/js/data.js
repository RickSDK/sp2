...
function getSuperpowersData() {
	...
	obj.campaigns = getCampaigns();
	return obj;
}
function getCampaigns() {
	return [
		{ id: 1, name: 'Combat Training', capitalsWin: 2, lock: false, currentFlg: true, unit: 3, type: 'freeforall', numPlayers: 2, desc: 'Learn the basic elements of the game as you attempt to take over Russia. Winning this unlocks Tutorial #2.', mission: 'Begin the conquest! It\'s time to expand your empire. Click on Ukraine to invade.' },
		...
		{ id: 4, name: 'Nuke Tutorial', capitalsWin: 2, lock: true, currentFlg: false, unit: 14, type: 'freeforall', numPlayers: 2, desc: 'Global Thermal Nuclear War!', mission: 'This tutorial is simple. Spend 4 rounds buying and nuking your enemy. Read up on nukes so you know how they work.' },
		{ id: 5, name: 'Ship Tutorial', capitalsWin: 2, lock: true, currentFlg: false, unit: 12, type: 'freeforall', numPlayers: 2, desc: 'Learn to load, move and attack with ships.', mission: 'In this tutorial you will learn how to load troops onto a transport, move accross the ocean and mount an amphibious assault.' },
		...
		{ id: 9, name: 'Hard Fog', campaignFlg: true, capitalsWin: 5, lock: true, currentFlg: false, unit: 6, type: 'diplomacy', numPlayers: 4, desc: 'You thought fog of war was tough, wait until you play with hard fog.', mission: 'Hard fog has even more information hidden from you. Team up and control 5 capitals to win the game.' },
		{ id: 10, name: 'Free-For-All', campaignFlg: true, capitalsWin: 6, lock: true, currentFlg: false, unit: 2, type: 'freeforall', numPlayers: 5, desc: 'It\'s you against the world in a free for all game with hard fog.', mission: 'This will be your hardest callenge yet! Hard fog and no teams. It is you against the world. First player to control 6 capitals wins the game.' },
		{ id: 11, name: 'World-War-II', campaignFlg: true, capitalsWin: 6, lock: true, currentFlg: false, unit: 2, type: 'ww2', numPlayers: 4, desc: 'World War 2 simulation using Superpowers map and territories.' },
	];
}
function populateUnits() {
	...
	units.push({ id: 20, name: 'Humvee', type: 1, subType: 'vehicle', target: 'soldierOnly', cost: 4, move: 2, att: 2, numAtt: 1, def: 5, numDef: 1, cas: 5, flag: 1, targetDroneFlg: true, desc: ['Cheap, mobile units with a strong defense against soldiers but cannot target other types of units.'] });
	...
	units.push({ id: 28, name: 'Medic', type: 1, subType: 'soldier', cost: 4, move: 1, att: 1, numAtt: 1, def: 1, cas: 5, flag: 1, desc: ['Heals 1 infantry or soldier casualty per battle on offense or defense.', 'Medics do not heal against cruise missiles, nukes or EMPs.'] });
	...
	units.push({ id: 36, name: 'Apache', type: 4, subType: 'chopper', cost: 10, move: 2, att: 5, numAtt: 1, def: 3, cas: 10, flag: 1, desc: ['Apache helicopters attack in air, sea and land. They can occupy and hold a land territory by themselves.', 'Get extra attack dice with Anthrax tech, and face at most 1 Air Defense with Stealth. Also target tanks with AGM Maverick Missiles tech. They no longer get extra range.'] });
	units.push({ id: 37, name: 'Striker', type: 1, subType: 'vehicle', target: 'planesTanks', cost: 10, move: 2, att: 4, numAtt: 1, def: 4, numDef: 3, cas: 10, flag: 2, targetDroneFlg: true, desc: ['Strikers are strong attack and even stronger defensive units that will target planes and vehicles first. Out of the box you get 1 dice on attack and 3 dice on defense.', 'Comes with AA gun mounted on top.', 'Extra attack dice with Anthrax tech.'] });
	...
	units.push({ id: 43, name: 'Predator Drone', type: 2, subType: 'fighter', cost: 8, move: 4, att: 3, numAtt: 2, def: 1, cas: 10, flag: 8, returnFlg: true, desc: ['Predator Drones are great for scoring free kills against weakly defended land territories. They can fly out 2 spaces and attack any land territory with 2 dice, needing 3 or less to hit. They can ONLY be shot down by Air Defense and fighters. Drones only attack on the first round of battle, then fly home.', 'Any fighter defending against a drone will automatically score a hit.', 'With Stealth they are immune to air defense, and can only be hit by fighters. They get upgraded to target tanks and artillery (not special units) with AGM Maverick Missiles technology. Also range is extended to 3 spaces with Long Range Aircraft tech.', 'When defending, they can be hit by any unit.'] });
	...
	units.push({ id: 48, name: 'J-20', type: 2, subType: 'fighter', cost: 12, move: 4, att: 4, numAtt: 1, def: 4, cas: 12, flag: 5, returnFlg: false, desc: ['J-20s are advanced powerful attack fighters.', 'They defend at 5 with stealth, target tanks with AGM Maverick Missiles and target planes with Advanced Radar.', 'Also increased range with technology and 2 dice with Nuclear Warheads.'] });
...
function getGameTerritories(type) {
	if(type==2)
		return getGameTerritoriesAA();
	var terrs = [];
	var x = 1;
	...
	terrs.push({ id: x++, name: 'Germany', x: 637, y: 217, nation: 2, owner: 0, type: 0, capital: true, water: 110, land: [8, 12, 62], enemyWater: 110, enemyZone: 10, enemyWater2: 106, enemyZone2: 58, borders: '110+113+62+12+8' });
	terrs.push({ id: x++, name: 'France', x: 575, y: 255, nation: 2, owner: 0, type: 0, water: 0, borders: '109+7+12+114+9', land: [7, 9, 12] });
	terrs.push({ id: x++, name: 'Spain', x: 531, y: 297, nation: 2, owner: 0, type: 0, water: 0, borders: '109+105+114+8', land: [8] });
	terrs.push({ id: x++, name: 'United Kingdom', x: 554, y: 166, nation: 2, owner: 0, type: 0, water: 0, borders: '109+110', land: [] });
	terrs.push({ id: x++, name: 'Sweden Finland', x: 665, y: 84, nation: 2, owner: 0, type: 0, water: 0, borders: '113+61+14+111', land: [14, 61] });
	...
	terrs.push({ id: x++, name: 'Caucasus', x: 787, y: 191, nation: 3, owner: 0, type: 0, water: 0, borders: '14+15+63+17+13', land: [14, 15, 13, 17, 63] });
	...
	terrs.push({ id: x++, name: 'Okhotsk', x: 1126, y: 203, nation: 4, owner: 0, type: 0, water: 0, borders: '22+20+24+141', land: [22, 20, 24] });
	terrs.push({ id: x++, name: 'Korea', x: 1100, y: 258, nation: 4, owner: 0, type: 0, water: 0, borders: '20+23+141+21+25+30+64', land: [20, 23, 64, 30, 25, 21] });
	...
	terrs.push({ id: x++, name: 'Turkey', x: 725, y: 315, nation: 6, owner: 0, type: 0, water: 0, borders: '12+116+63+38+37+115', land: [12, 63, 37, 38] });
	...
	terrs.push({ id: x++, name: 'Afghan-Pakistan', x: 881, y: 379, nation: 6, owner: 0, type: 0, water: 0, borders: '17+38+70+120', land: [38, 17, 70] });
	...
	terrs.push({ id: x++, name: 'Congo', x: 665, y: 541, nation: 7, owner: 0, type: 0, capital: true, water: 96, land: [44, 45, 46, 48, 73], enemyWater: 98, enemyZone: 50, enemyWater2: 104, enemyZone2: 43, borders: '96+73+48+46+45+44' });
	terrs.push({ id: x++, name: 'West Africa', x: 540, y: 442, nation: 7, owner: 0, type: 0, water: 0, borders: '104+74+45+44+75', land: [74, 75, 44, 45] });
	terrs.push({ id: x++, name: 'Nigeria', x: 609, y: 495, nation: 7, owner: 0, type: 0, water: 0, borders: '75+43+45+42+96', land: [75, 43, 45, 42] });
	terrs.push({ id: x++, name: 'Chad', x: 652, y: 453, nation: 7, owner: 0, type: 0, water: 0, borders: '43+74+41+46+42+44', land: [43, 44, 74, 41, 46, 42] });
	terrs.push({ id: x++, name: 'Sudan', x: 705, y: 472, nation: 7, owner: 0, type: 0, water: 0, borders: '118+41+40+47+48+42+45', land: [45, 42, 48, 47, 40, 41] });
	...
	terrs.push({ id: x++, name: 'South Africa', x: 686, y: 700, nation: 7, owner: 0, type: 0, water: 0, borders: '73+48+72+123', land: [72, 73, 48] });
	...
	terrs.push({ id: x++, name: 'Argentina', x: 342, y: 635, nation: 8, owner: 0, type: 0, water: 0, borders: '91+54+77+92+93+50', land: [50, 54, 77] });
	...
	terrs.push({ id: x++, name: 'Norway', x: 624, y: 97, nation: 0, owner: 0, type: 0, water: 0, borders: '111+110+11', land: [11] });
	terrs.push({ id: x++, name: 'Ukraine', x: 702, y: 228, nation: 0, owner: 0, type: 0, water: 0, borders: '113+7+12+116+15+14', land: [7, 12, 14, 15] });
	terrs.push({ id: x++, name: 'Georgia', x: 772, y: 265, nation: 0, owner: 0, type: 0, water: 0, borders: '116+15+16+17+117+38+36', land: [15, 16, 17, 36, 38] });
	...
	terrs.push({ id: x++, name: 'India', x: 935, y: 404, nation: 0, owner: 0, type: 0, water: 0, borders: '120+39+29+28+17', land: [39, 17, 29, 28] });
	terrs.push({ id: x++, name: 'Madagascar', x: 788, y: 645, nation: 0, owner: 0, type: 0, water: 0, borders: '122+125+127', land: [] });
	terrs.push({ id: x++, name: 'Mozambique', x: 714, y: 640, nation: 0, owner: 0, type: 0, water: 0, borders: '48+122+49', land: [48, 49] });
	terrs.push({ id: x++, name: 'Angola', x: 648, y: 629, nation: 0, owner: 0, type: 0, water: 0, borders: '42+48+49+95+96', land: [42, 48, 49] });
