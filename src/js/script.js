...
function showUnitsForMovementBG(type, units, currentPlayer, territories, selectedTerritory, goButton, round, currentPlayer, optionType, svgs) {
	...
	units.sort(function(a, b) { return a.terr - b.terr; });
	obj.moveTerr = [];
	var tUnits = [];
	var terId=0;
	var maxDist = 2;
	...
	for(var x=0; x<units.length; x++) {
		...
		if(unit.owner==currentPlayer.nation && unit.mv>0 && (unit.movesLeft>0 || unit.terr>=79)) {
			...
			var movement = (type=='attack' || type=='nuke')?unit.moveAtt:unit.mv;
			...
			if(unit.terr == terId) {
				...
				if(showSelectableBox(unit, territories[terId-1], selectedTerritory, type, goButton, round, currentPlayer)) {
					tUnits.push(unit);
				}
			} else	{
				if(terId>0) {
					var unitTerr = territories[terId-1];
					distObj = distanceBetweenTerrs(unitTerr, selectedTerritory, maxDist, 0, 0, 0, allyHash, territories);
					if(showConsole)
						console.log('xxx', maxDist, unitTerr.name, distObj, tUnits);
					if(includeThisTerritoryForDisplay(type, unitTerr, selectedTerritory, distObj)) {
	...
	if(tUnits.length>0) {
		distObj = distanceBetweenTerrs(territories[terId-1], selectedTerritory, maxDist, 0, 0, 0, allyHash, territories);
		var unitTerr = territories[terId-1];
		if(includeThisTerritoryForDisplay(type, unitTerr, selectedTerritory, distObj)) {
			var nUnits=numberOfUnitsThatCanReach(distObj, tUnits, optionType);
			if(nUnits>0) {
				totalUnitsThatCanMove += nUnits;
				obj.moveTerr.push({id: territories[terId-1].id, nation: territories[terId-1].nation, name: territories[terId-1].name, distObj: distObj, units: tUnits});
			}
		}
	}
